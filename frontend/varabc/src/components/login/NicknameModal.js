    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center z-50">
        ...
        <div className="bg-white p-8 rounded shadow-lg z-50">
          ...
          />
          <button className="mt-4 px-4 py-2 bg-red-500 text-white rounded" onClick={checkNickname}>중복 검사</button>
          {NicknameCheck ? <p>사용 가능</p> : <p>사용 불가</p>}
          <button className="mt-4 px-4 py-2 bg-red-500 text-white rounded" onClick={handleSave}>확인</button>
        </div>
      </div>
    );
  }
